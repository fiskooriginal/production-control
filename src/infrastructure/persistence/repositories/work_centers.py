...
class WorkCenterRepository(WorkCenterRepositoryProtocol):
    ...
    async def create(self, domain_entity: WorkCenterEntity, author: UUID | None = None) -> WorkCenterEntity:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка целостности данных при создании рабочего центра: {e}") from e
        except DBAPIError as e:
            raise DatabaseException(f"Ошибка базы данных при создании рабочего центра: {e}") from e
        except SQLAlchemyError as e:
            raise DatabaseException(f"Ошибка SQLAlchemy при создании рабочего центра: {e}") from e
        except Exception as e:
            raise MappingException(f"Ошибка маппинга при создании рабочего центра: {e}") from e
    ...
    async def get(self, uuid: UUID) -> WorkCenterEntity | None:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка SQLAlchemy при получении рабочего центра: {e}") from e
        except Exception as e:
            raise MappingException(f"Ошибка маппинга при получении рабочего центра: {e}") from e
    ...
    async def update(self, domain_entity: WorkCenterEntity) -> WorkCenterEntity:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка целостности данных при обновлении рабочего центра: {e}") from e
        except DBAPIError as e:
            raise DatabaseException(f"Ошибка базы данных при обновлении рабочего центра: {e}") from e
        except SQLAlchemyError as e:
            raise DatabaseException(f"Ошибка SQLAlchemy при обновлении рабочего центра: {e}") from e
        except Exception as e:
            raise MappingException(f"Ошибка маппинга при обновлении рабочего центра: {e}") from e
    ...
    async def delete(self, uuid: UUID) -> None:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка целостности данных при удалении рабочего центра: {e}") from e
        except DBAPIError as e:
            raise DatabaseException(f"Ошибка базы данных при удалении рабочего центра: {e}") from e
        except SQLAlchemyError as e:
            raise DatabaseException(f"Ошибка SQLAlchemy при удалении рабочего центра: {e}") from e
    ...
    async def exists(self, uuid: UUID) -> bool:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка SQLAlchemy при проверке существования рабочего центра: {e}") from e
    ...
    async def list(
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка SQLAlchemy при получении списка рабочих центров: {e}") from e
        except Exception as e:
            raise MappingException(f"Ошибка маппинга при получении списка рабочих центров: {e}") from e
    ...
    async def get_by_identifier(self, identifier: str) -> WorkCenterEntity | None:
        ...
        try:
            ...
            raise DatabaseException(f"Ошибка SQLAlchemy при поиске рабочего центра по идентификатору: {e}") from e
        except Exception as e:
            raise MappingException(f"Ошибка маппинга при поиске рабочего центра по идентификатору: {e}") from e
